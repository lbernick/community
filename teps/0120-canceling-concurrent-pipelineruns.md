...
The following alternatives are identified by 1. where concurrency controls are defined,
and 2. which controller is responsible for handling the concurrency controls.
Deciding on a specific syntax for concurrency controls is not the goal of this section.

### PipelineRun controller handles concurrency controls

In the following solutions, the PipelineRun controller would apply a concurrency key
to PipelineRuns it creates, either as a label or as part of the PipelineRun status.
When reconciling a PipelineRun, it would need to determine which other PipelineRuns are part
of the same concurrency group, either by querying by label selector or filtering PipelineRuns from
the informer. It would then cancel all PipelineRuns except the one most recently created.

**TODO: Insert detail about avoiding race condition**
...
In this solution, concurrency would be defined on a Pipeline. Any PipelineRuns that reference this
Pipeline, or have it embedded in their spec, will be considered part of the same concurrency group
(even if they are in different namespaces).

Pros:
- Available cancelation strategies can be the same as what's defined on Pipeline (FIXME)
...
In this solution, concurrency would be defined on a Pipeline. Any PipelineRuns with the same concurrency
key, regardless of which Pipeline they reference, will be considered part of the same concurrency group
(even if they are in different namespaces).

If two PipelineRuns have the same key but different concurrency strategies,
reconciliation will fail. This solution assumes that PipelineRuns using concurrency
will typically be created by tooling such as a trigger, Pipelines as Code, a workflow, etc,
and would likely not have different concurrency strategies.

If we want to ensure that PipelineRuns referencing different Pipelines don't accidentally
have the same concurrency key, we can hash the Pipeline spec and generate a new, deterministic
concurrency key based on the one defined in the PipelineRun spec and the hash of the Pipeline spec,
and put this concurrency key in the PipelineRun status or as a label.
...
In this solution, a configMap defines concurrency controls for PipelineRuns. This configMap could
be cluster-wide, and apply to all PipelineRuns in the cluster, or namespaced, and apply to all
PipelineRuns in the same namespace (or we could have both).
...
data:
  ...
  - name: pipelinerun-pull-requests
    ...
    - input: “ci-pipeline”
      operator: in
      values: [“$(spec.pipelineRef.name)”, “$(spec.pipelineSpec.name)”]
    key: $(metadata.namespace)-$(spec.params["pull-request"])
    strategy: cancelRunFinally
```

When creating a PipelineRun, the PipelineRun controller would need to check each of the
concurrency rules and determine which of the rules it matches. For each matching rule, it would
compute the concurrency key and add it as a label to the PipelineRun.

This solution implies that PipelineRuns may belong to multiple concurrency groups.
(We could explore a solution where concurrency rules are defined in a configMap but we restrict
the configuration to one rule per Pipeline.)
If a PipelineRun has multiple concurrency keys, any running PipelineRuns that have a matching
concurrency key will be canceled.

Pros:

Cons:
- if you want to put pull request in labels, you have to change PR spec.

### Concurrency handled in Triggers
...
As in other strategies, the EventListener will create PipelineRuns with concurrency keys as labels.
The generated concurrency key should include the EventListener name, so one EventListener cannot
cancel PipelineRuns created by another EventListener.

When an EventListener receives an event, it will query for all PipelineRuns with the appropriate
concurrency key and cancel all of them (there should be at most one).
After issuing all cancellation requests, it should create the new PipelineRun.
...
### New concurrency controller and CRD

In this solution, a new concurrency CRD can be applied to a namespace to control PipelineRun execution
...
The concurrencyControl controller would be responsible for reconciling PipelineRuns.
When a new PipelineRun is created, the controller would need to query all concurrencyControl CRDs
to determine whether the PipelineRun matches any of them.
For each matching concurrencyControl, the controller would add a concurrency key label
to the PipelineRun. It would filter for any PipelineRuns with matching concurrencyKeys and cancel
all of them.

This solution is similar to storing concurrency controls in a configmap and implementing them
in the PipelineRun reconciler, except that the logic is moved to a separate reconciler and controls
to separate CRDs.

### Workflows controller and CRD
